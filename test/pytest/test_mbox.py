...
def test_hpps_to_trch_with_no_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'none', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_trch_with_select_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'select', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_trch_with_poll_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'poll', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_trch_with_epoll_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'epoll', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_rtps_with_no_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'none', '-o', '/dev/mbox/1/mbox0', '-i', '/dev/mbox/1/mbox1', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_rtps_with_select_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'select', '-o', '/dev/mbox/1/mbox0', '-i', '/dev/mbox/1/mbox1', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_rtps_with_poll_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'poll', '-o', '/dev/mbox/1/mbox0', '-i', '/dev/mbox/1/mbox1', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_hpps_to_rtps_with_epoll_notification_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-n', 'epoll', '-o', '/dev/mbox/1/mbox0', '-i', '/dev/mbox/1/mbox1', '-c', str(core_num)])
    assert out.returncode == 0

@pytest.mark.parametrize('core_num', range(8))
def test_invalid_outbound_mailbox_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
    out = run_tester_on_host(host, 0, [], ['-c', str(core_num), '-o', '32'])
    assert out.returncode == 1

@pytest.mark.parametrize('core_num', range(8))
def test_invalid_inbound_mailbox_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
...
@pytest.mark.parametrize('core_num', range(8))
def test_early_timeout_on_each_core(qemu_hpps_ser_conn_per_mdl, host, core_num):
...
def test_multiple_cores_same_mbox(qemu_hpps_ser_conn_per_mdl, host):
    out = run_tester_on_host(host, 1, [], ['-C', '0', '-c', '1'])
    assert out.returncode == 0

def test_multiple_cores_same_mbox_invalid_CPU1(qemu_hpps_ser_conn_per_mdl, host):
    out = run_tester_on_host(host, 1, [], ['-C', '-1', '-c', '1'])
    assert out.returncode == 1

def test_multiple_cores_same_mbox_invalid_CPU2(qemu_hpps_ser_conn_per_mdl, host):
