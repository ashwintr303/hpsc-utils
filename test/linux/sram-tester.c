...
static void modify_array(int *arr, int num_ints) {
    int i;
...
static int access_sram(int num_ints, int is_modify) {
    int fd;
    int file_size;
    ...
    */
    ...
    if (is_modify) {
	/* Write to the array to see if the changes are visible later. */
        modify_array(map, num_ints);
    }
...
static void usage(const char *pname, int code) {
    fprintf(code ? stderr : stdout,
            "Usage: %s [-s SIZE] [-m] [-h]\n"
            "  -s, --size=SIZE      The array size (in ints) to modify/print (default=" num2str(DEFAULT_SIZE) ")\n"
            "  -m, --modify         Modify the first SIZE ints in the array\n"
            "  -h, --help           Print this message and exit\n",
            pname);
    exit(code);
}

static const char short_options[] = "s:mh";
...
int main(int argc, char *argv[]) {
    ...
    ret = access_sram(num_ints, is_modify);
    return ret;
}
